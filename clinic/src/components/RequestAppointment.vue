<template>
  <div class="createNew container">
     ...
     <form v-on:submit="createAppointment">
        <div class="well">
  ...
  methods: {
        createAppointment(e){
         if(!this.patient.first_name || !this.patient.last_name || !this.patient.phone_number || !this.patient.doctor_name){
                ...
                this.$http.post('http://clinicserver/api/appointment/new', newAppointment)
                    .then(function(response){
                        this.$router.push({path: '/viewall', query: {alert: 'New Appointment is Added'}});
                    });
                e.preventDefault();
            }
            e.preventDefault();
